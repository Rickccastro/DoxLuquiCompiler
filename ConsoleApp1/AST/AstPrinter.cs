...
namespace ConsoleApp1.AST
{
    ...
    // ------------------------------------------------------------
    // Percorre a árvore e imprime de forma indentada, para ver
    // a estrutura que o parser construiu. Não faz parte da
    // compilação em si — é uma ferramenta de depuração.
    public static class AstPrinter
    {
        ...
        private static void PrintExpr(Expr expr, int indent, StringBuilder sb)
        {
            switch (expr)
            {
                ...
                case UnaryExpr u:
                    Line(sb, indent, $"Unary {u.Op.Lexema}");
                    PrintExpr(u.Operand, indent + 1, sb);
                    break;

                case BinaryExpr bin:
                    Line(sb, indent, $"Binary {bin.Op.Lexema}");
