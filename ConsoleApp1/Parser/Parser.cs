...
namespace ConsoleApp1.Parser
{
    ...
    public class Parser
    {
        ...
        private Stmt VarDeclaration()
        {
            ...
            return new VarDeclStmt(typeToken.Type, name.Lexema, initializer, name.Linha);
        }
        ...
        private Stmt Assignment()
        {
            ...
            return new AssignStmt(name.Lexema, value, name.Linha);
        }

        // "if" "(" expression ")" statement ("else" statement)?
        private Stmt IfStatement()
        {
            int Linha = Previous().Linha; // linha do token 'if'
            ...
            return new IfStmt(condition, thenBranch, elseBranch, Linha);
        }

        // "while" "(" expression ")" statement
        private Stmt WhileStatement()
        {
            int Linha = Previous().Linha; // linha do token 'while'
            Consume(TokenType.LParen, "Esperava '(' depois de 'while'");
            Expr condition = Expression();
            Consume(TokenType.RParen, "Esperava ')' depois da condição");
            Stmt body = Statement();
            return new WhileStmt(condition, body, Linha);
        }
        ...
        private Stmt ReadStatement()
        {
            ...
            return new ReadStmt(name.Lexema, name.Linha);
        }
        ...
        private Expr Primary()
        {
            ...
            {
                Token id = Previous();
                return new VariableExpr(id.Lexema, id.Linha);
            }
        ...
        {
            string found = token.Type == TokenType.EOF ? "o fim do arquivo" : $"'{token.Lexema}'";
            return new ParseError(token.Linha, $"{message} (encontrei {found})");
        }
    }
}
